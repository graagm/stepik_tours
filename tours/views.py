...
ran_tours_set = set(random.randint(1,16) for _ in range(6))

while len(ran_tours_set) < 6:
    ran_tours_set.add(random.randint(1, 16))
    
ran_tours = list(ran_tours_set)
tours_new = {i:tours[i] for i in ran_tours}

for i in tours_new.values():
    i['stars'] = int(i['stars']) * '*'
    
min_price = min([int(i['price']) for  i in tours_new.values()])
max_price = max([int(i['price']) for  i in tours_new.values()])
min_nights = min([int(i['nights']) for  i in tours_new.values()])
max_nights = max([int(i['nights']) for  i in tours_new.values()])


def main_view(request):
    return render(request, 'index.html', {'tours_new' : tours_new})


def departure_view(request, city):
    return render(request, 'departure.html', {'tours_new' : tours_new, "city":city, 'departures':departures, 'min_price':min_price, 'max_price':max_price, 'min_nights':min_nights, 'max_nights':max_nights})

def tour_view(request, hotel_number, city=0):
    return render(request, 'tour.html', {'departures':departures, 'tours_new' : tours_new, 'hotel_number':hotel_number, "city":city,})
